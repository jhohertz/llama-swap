...
func serveCmd(args []string) error {
	...
	fs.StringVar(&f.healthPath, "health-path", "/health", "backend health endpoint")
...
// handler gates requests on backend readiness, then proxies.
func (s *server) handler(w http.ResponseWriter, r *http.Request) {
	if !s.ready.Load() {
		reason := s.readyReason.Load().(string)
		// llama-swap polls this while loading; 503 means "not ready yet".
		// Direct client traffic gets the same answer with a reason.
